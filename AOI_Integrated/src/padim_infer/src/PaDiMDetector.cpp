...
#include <NvOnnxParser.h>  // 这是tensorRT的ONNX解析器
#include <vector>
#include <numeric>
#include <algorithm>
#include <chrono>     // 用于计时
...
class Logger : public nvinfer1::ILogger {    //日志类，用来接收tensorRT的日志信息
    void log(Severity severity, const char* msg) noexcept override {    //Severity是日志等级
        if (severity <= Severity::kWARNING) std::cout << "[TRT] " << msg << std::endl; //只打印警告及以上级别的日志
    }
} gLogger;//创建全局对象，供tensorRT使用

void check(cudaError_t result, char const *const func, const char *const file, int const line) {  // CUDA 错误检查函数
    if (result) {
...
bool PaDiMDetector::dumpNGImage(const cv::Mat& rawImg, float score,
                                const std::string& saveDir, const std::string& fileName) {
    if (rawImg.empty()) return false;

    // 1. 准备目录: Output/Pending/
    // 为了防止大量文件堆积，建议加一层 Pending 子目录
    std::string pendingDir = saveDir + "/Pending/";
    if (!fs::exists(pendingDir)) {
        fs::create_directories(pendingDir);
    }

    // 2. 构造文件名
    // 格式: {原文件名}__NG_{分数}.jpg
    // 例如: Component_01__NG_85.jpg
    // 使用分数作为后缀方便人工查看
    std::string outPath = pendingDir + fileName + "__NG_" + std::to_string((int)score) + ".jpg";

    // 3. 保存
    return cv::imwrite(outPath, rawImg);
}
